...
class FlightController():
    ...
    def deliver(self, payload):
        ...
        self.change_state("transit-to")
        self.go_to_helper(payload)
    ...
    def location_callback(self, vehicle, name, location):
        ...
        if self.state == 'transit-to':
            ...
            if distance < 2:
                ...
                self.change_state('transit_await')
                time.sleep(5)
                print("Waiting for pickup...")
                self.change_state('transit-deliverd')
                time.sleep(5)
                print('Delivered go back home..')
                self.change_state('transit-back')
                self.go_to_helper(
                    {'lat': constants.HOME_LAT, 'lon': constants.HOME_LNG})
